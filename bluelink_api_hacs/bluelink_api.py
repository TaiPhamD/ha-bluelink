...
class BluelinkAPI:
    ...
    def _refresh_token_if_needed(self):
        ...
        if self._token_expired():
            ...
            self._refresh_token()
            if self._auth.get("expires_at", 0) <= previous_expires_at:  # Check if expires_at was not updated
                # Attempt a full re-login if the refresh token did not update 'expires_at'
                self._auth = self.login()  
                if self._token_expired():  # Check again if the token is still expired after re-login
                    print("Error: Unable to refresh token or re-login.")
                    return False
        return True
    ...
    def start_climate(self, temp=72):
        if not self._refresh_token_if_needed():
            return "Authentication failed. Unable to start climate control."
        ...
        if response.status_code != 200:
            print(f"Error starting climate control: {response.status_code}")
            return None
    ...
    def stop_climate(self):
        if not self._refresh_token_if_needed():
            return "Authentication failed. Unable to stop climate control."
     
        url = f"{BASE_URL}/ac/v2/evc/fatc/stop"
        headers = self._set_request_headers()
        response = self._session.post(url, headers=headers)
        if response.status_code != 200:
            print(f"Error stopping climate control: {response.status_code}")
            return None
        return response